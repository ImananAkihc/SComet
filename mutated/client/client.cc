    ...
    uint64_t bytes, bool measure)
{
    ...
    if (rcvd_count_ % int(cfg_.req_s) == 0) {
        ...
        for (double my_percent = 0.05; my_percent < 1; my_percent += 0.05)
            printf("%dth percentile latency %f\n", int(my_percent * 100), double(results_.service().percentile_histogram(my_percent, int(10 * cfg_.req_s)) + results_.queue().percentile_histogram(my_percent, int(10 * cfg_.req_s))));
        for (double my_percent = 0.99; my_percent < 1; my_percent += 0.0005)
            printf("%.2fth percentile latency %f\n", my_percent * 100, double(results_.service().percentile_histogram(my_percent, int(10 * cfg_.req_s)) + results_.queue().percentile_histogram(my_percent, int(10 * cfg_.req_s))));
        cout << endl;
